...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def forward_features(self, x, mask, with_last_n=None):
        ...
        for i, blk in enumerate(self.blocks):
            x_vis = blk(x_vis)
            if (with_last_n is not None) and (len(self.blocks) - i <= with_last_n):
                feat.append(self.norm(x_vis.mean(1)))

        x_vis = self.norm(x_vis)
        if with_last_n is None:
            return x_vis
        else:
            return x_vis, feat
    ...
    def get_intermediate_layers(self, x, n=1):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if len(self.blocks) - i <= n:
                output.append(self.norm(x.mean(1)))
        return output

    def forward(self, x, mask, with_last_n=None):
        x = self.forward_features(x, mask, with_last_n)
        if with_last_n is not None:
            x, feat = x
        x = self.head(x)
        if with_last_n is None:
            return x
...
class PretrainVisionTransformer(nn.Module):
    ...
    def forward(self, x, mask, with_last_n=None):
        ...
        x_vis = self.encoder(x, mask, with_last_n=with_last_n) # [B, N_vis, C_e]
        if with_last_n is not None:
            x_vis, feat = x_vis
...
def pretrain_mae_small_patch16_112(pretrained=False, **kwargs):
