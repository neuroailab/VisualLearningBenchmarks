...
def make_sparse(out, sparsity):
    if sparsity is None:
        return out
    assert out.dim() == 4
    ave_out = torch.mean(out, dim=(-2, -1))
    mask = torch.zeros_like(ave_out)
    _, indices = torch.topk(
            ave_out, k=int(sparsity*ave_out.size(1)), 
            dim=1, sorted=False)
    mask = mask.scatter(1, indices, 1)
    mask = mask.unsqueeze(-1).unsqueeze(-1)
    mask = torch.tile(mask, (1, 1, out.size(-2), out.size(-1)))
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out += identity
        out = self.relu(out)
        out = make_sparse(out, self.sparse_cfg)
...
class ResNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 per_stage_sparse=[None, None, None, None],
        ...
        self.per_stage_sparse = per_stage_sparse
        ...
        if add_neck is not None:
            self.neck = builder.build_neck(add_neck)
            if 'out_channels' in add_neck:
                self.feat_dim = add_neck['out_channels']
        else:
            self.neck = None
