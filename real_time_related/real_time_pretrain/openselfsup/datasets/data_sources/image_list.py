...
NEW_DATA_ROOT = os.environ.get('NEW_DATA_ROOT', None)
...
class ImageList(object):
    ...
    def __init__(self, root, list_file, memcached=False, mclient_path=None):
        if not os.path.isdir(root) and NEW_DATA_ROOT is not None:
            root = NEW_DATA_ROOT
    ...
    def load_one_img_from_path(self, path):
        if self.memcached:
            self._init_memcached()
        if self.memcached:
            img = self.mc_loader(path)
        else:
            img = Image.open(path)
        img = img.convert('RGB')
        return img
    ...
    def get_sample(self, idx):
        img = self.load_one_img_from_path(
                self.fns[idx])
        if self.has_labels:
