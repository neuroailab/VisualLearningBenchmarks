...
class LrUpdaterHook(Hook):
    ...
    def __init__(self,
        ...
        if warmup is not None:
            if warmup not in ['constant', 'linear', 'exp', 'square']:
                raise ValueError(
    ...
    def _set_lr(self, runner, lr_groups):
        if isinstance(runner.optimizer, dict):
            ...
            for param_group, lr in zip(runner.optimizer.param_groups,
                                       lr_groups):
                if 'use_fix_lr' not in param_group:
                    param_group['lr'] = lr
                else:
                    param_group['lr'] = param_group['use_fix_lr']
    ...
    def get_warmup_lr(self, cur_iters):
        ...
        elif self.warmup == 'square':
            k = (1 - (cur_iters / self.warmup_iters) ** 2) * (1 - self.warmup_ratio)
            warmup_lr = [_lr * (1 - k) for _lr in self.regular_lr]
