...
MODEL_SAVE_FOLDER = os.environ.get(
        'MODEL_SAVE_FOLDER',
        f'/mnt/fs4/{USER_NAME}/openselfsup_models/work_dirs/new_pipelines')
...
class ParamsBuilder(object):
    ...
    def build_cifar_eval_data_loader(self):
        val_nn=dict(
                ...
                data_source=dict(
                    type='Cifar100Mix',
                    root=f'/data5/{USER_NAME}/Dataset/cifar100/'),
                pipeline=cifar_funcs.get_cifar_test_augs())
        self.cifar_val_dataset = build_dataset(val_nn)
        return self.build_val_loader_from_dataset(val_dataset=self.cifar_val_dataset)
